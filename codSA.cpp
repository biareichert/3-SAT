...
/* Anotações
	...
	Fazer:
		Escolher uma função de resfriamento
		Testar!!

	Compilação e execução exemplo:
	$ g++ -Wall -O3 -o e codSA.cpp
	$ ./e uf20-91-01.cnf saida 0 1 0.0001
...
double formulaResfrimento (int iteracao_atual){
    ...
    //return ((temperatura_inicial - temperatura_final) / (cosh((10.0 * iteracao_atual) / qtd_iteracoes)) + temperatura_final);
}
...
vector<bool> gerarNovaConfiguracaoVariveis(vector<bool> &configuracao_atual){
	...
	for(int i = 0; i < configuracao_atual.size(); i++){
		...
		if(aleatorio < 0.005){ //chance de 1% de uma mudar a configuração de cada variável, basicamente. Ver isso dps.
...
int main(int argc, char const *argv[]) {
	...
	//Verificamos se todos os parâmetros foram devidamente informados, se nã oretorna a descirção correta
	if (argc != 5){
		printf("4 argumentos: <arquivoEntrada> <arquivo saída sem formato> <temperatura_inicial> <temperatura_final> \n");
		return 1;
	}
	...
	for(i = 0; i < numero_clausulas; i++){ 
		...
		Clausulas.push_back(Clausula(vet)); //Insiros esse meu vetor de 3 inteiros como uma claúsulas no meu vector de cláusulas
    }
	...
	for(int j=0; j<numero_execucoes; j++){
		...
		arqSaida = argv[2] + to_string(j+1) + formato; //altero o nome para saída.
		//cout << arqSaida << endl;
		...
		do {
			...
			if(delta > 0){ //Se a nova configuração é mais satisfatoria em clausula, atribuo ela
				...
				//Calculo da função de probabilidade. No pseucodódigo
		...
		if(temperatura_atual == temperatura_final || qtd_iteracoes == i + 1)
			cout << "Temperatura/Iterações limite alcançada!" << endl;

...
int main(int argc, char const *argv[]) {
	...
	for(int j=0; j<numero_execucoes; j++){
		...
		vector<bool> S_out = gerarConfiguracaoAleatoria(numero_variaveis);
		int S_out_FO = verificaQuantidadeClausulasSatisfeitas(Clausulas, S_out);

		configuracao_atual = S_out;
		clausulas_satisfeitas = S_out_FO;
		...
		arqSaida = "saidaRS_" + to_string(j+1) + formato; //altero o nome para saída.
		arquivo_saida.open(arqSaida);
		
		for(int i = 1; i <= qtd_iteracoes; i++){
			//Vai iterando e gerando configurações aleatórios, a fim de ver se alguma se torna melhor
			vector<bool> S = gerarConfiguracaoAleatoria(numero_variaveis);
			int S_FO = verificaQuantidadeClausulasSatisfeitas(Clausulas, S);
			if(S_FO > S_out_FO){
				S_out = S;
				S_out_FO = S_FO;
			}
			if(S_out_FO > clausulas_satisfeitas){
				configuracao_atual = S_out;
				clausulas_satisfeitas = S_out_FO;
			}
		...
		cout << "Satisfeitas " << clausulas_satisfeitas << " de " << numero_clausulas << " clausulas." << endl << endl;
